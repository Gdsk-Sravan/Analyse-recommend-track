...
# Phase C7g (2026-07-10): FRESH_START awareness. shadow_weekly runs on a
# separate Saturday workflow, so it wasn't wired to the env var previously.
# It reads shadow_trades.csv — which is deleted by main.py on fresh-start —
# and would emit an empty/misleading report if it ran anyway.
...
def _peek_fresh_start_marker(today_str: str = None) -> bool:
    ...
    Returns True iff a marker for TODAY is present. Never deletes — main.py
    cleans up on the next non-fresh-start run."""
...
def generate_weekly_report(csv_path: str = None,
    ...
    # Phase C7g (2026-07-10): FRESH_START gate. shadow_trades.csv was wiped
    # by main.py earlier today; running the rollup on an empty file would
    # commit a misleading "all buckets: n=0" report.
    if FRESH_START or _peek_fresh_start_marker():
        ...
        msg = f"fresh-start day ({_reason}) — skipping weekly rollup, no historical baseline"
        if not quiet:
            print(f"[FRESH_START] shadow_weekly: {msg}")
        return {"ok": False, "error": msg, "skipped_fresh_start": True}
...
if __name__ == "__main__":
    ...
    if result.get("skipped_fresh_start"):
        print(f"\nSKIPPED (fresh-start): {result.get('error', '')}")
    elif result["ok"]:
